...
- (int)nodeCountOfTag {
    NSString *jsString = [NSString stringWithFormat:@"document.images.length"];
    int len = [[self stringByEvaluatingJavaScriptFromString:jsString] intValue];
    return len;
}
...
- (NSArray *)getElementsByTagName:(NSString *)tagName
{
    ...
    for (int i = 0; i < [self nodeCountOfTag:tagName]; i++) {
        //获取xml某个节点下的所有内容（包括标签及内容）
        //NSString *jsString = [NSString stringWithFormat:@"document.getElementsByTagName('%@')[%d].innerHTML",tagName,i];
        NSString *jsString = [NSString stringWithFormat:@"document.getElementsByTagName('%@')[%d].getAttribute('class')",tagName,i];
        NSString *elementStr = [self stringByEvaluatingJavaScriptFromString:jsString];
        
        [elements addObject:elementStr];
    }
    
    return elements;
}

///  隐藏header 无header时根据标签div隐藏标题
- (NSString *)setHeaderHidden:(BOOL)hidden
{
    if (hidden) {        
        NSString *jsString = [NSString stringWithFormat:@"document.getElementsByTagName('header')[%d].style.display = 'none'",0];
        NSString *res = [self stringByEvaluatingJavaScriptFromString:jsString];
        
        if (res.length == 0) {
            jsString = [NSString stringWithFormat:@"document.getElementsByTagName('div')[%d].style.display = 'none'",1];
            res = [self stringByEvaluatingJavaScriptFromString:jsString];
        }
        return res;
    }else {
        return nil;
...
//        DLog(@"设置宽度%@",result);
//    }
    for (int i = 0; i < [self nodeCountOfTag]; i++) {
        NSString *jsString = [NSString stringWithFormat:@"document.getElementsByTagName('img')[%d].width = '%d'", i, size];
...
- (void)fillRectOnCanvas:(NSString *)canvasId x:(NSInteger)x y:(NSInteger)y width:(NSInteger)width height:(NSInteger) height uicolor:(UIColor *)color
{
    ...
    NSString *jsString = [NSString stringWithFormat:
                          ...
                          ,canvasId, [color webColorString], x, y, width, height];
    [self stringByEvaluatingJavaScriptFromString:jsString];
}
...
- (void)strokeRectOnCanvas:(NSString *)canvasId x:(NSInteger)x y:(NSInteger)y width:(NSInteger)width height:(NSInteger) height uicolor:(UIColor *)color lineWidth:(NSInteger)lineWidth
{
    NSString *jsString = [NSString stringWithFormat:
                          ...
                          ,canvasId, [color webColorString], lineWidth, x, y, width, height];
    [self stringByEvaluatingJavaScriptFromString:jsString];
}
...
- (void)arcOnCanvas:(NSString *)canvasId centerX:(NSInteger)x centerY:(NSInteger)y radius:(NSInteger)r startAngle:(float)startAngle endAngle:(float)endAngle anticlockwise:(BOOL)anticlockwise uicolor:(UIColor *)color
{
    NSString *jsString = [NSString stringWithFormat:
                          ...
                          "context.fillStyle = '%@';"
                          "context.fill();",
                          canvasId, x, y, r, startAngle, endAngle, anticlockwise ? @"true" : @"false", [color webColorString]];
    [self stringByEvaluatingJavaScriptFromString:jsString];
}
...
- (void)lineOnCanvas:(NSString *)canvasId x1:(NSInteger)x1 y1:(NSInteger)y1 x2:(NSInteger)x2 y2:(NSInteger)y2 uicolor:(UIColor *)color lineWidth:(NSInteger)lineWidth
{
    NSString *jsString = [NSString stringWithFormat:
                          ...
                          "context.lineWidth = %d;"
                          "context.stroke();",
                          canvasId, x1, y1, x2, y2, [color webColorString], lineWidth];
    [self stringByEvaluatingJavaScriptFromString:jsString];
}
...
- (void)linesOnCanvas:(NSString *)canvasId points:(NSArray *)points unicolor:(UIColor *)color lineWidth:(NSInteger)lineWidth
{
    ...
    jsString = [jsString stringByAppendingFormat:@""
                ...
                "context.lineWidth = %d;"
                "context.stroke();",
                [color webColorString], lineWidth];
    [self stringByEvaluatingJavaScriptFromString:jsString];
}
                  ...
                  lineWidth:(NSInteger)lineWidth
{
    NSString *jsString = [NSString stringWithFormat:
                          ...
                          "context.lineWidth = %d;"
                          "context.stroke();",
                          canvasId, x1, y1, cp1x, cp1y, cp2x, cp2y, x2, y2, [color webColorString], lineWidth];
    [self stringByEvaluatingJavaScriptFromString:jsString];
}
